...
export class BoxPlot extends React.Component {
  ...
  createBoxPlot() {
    return this.props.data.map(({ maximum, minimum, median, quartile1, quartile3, day }, i) =>
      ...
      <G x={this.xScale(day) + BoxPlot.axisYSize + (this.xScale.bandwidth() / 2)} y={this.yScale(median)}>
        <Line
        ...
        <Rect
          ...
          width={20}
          onMouseOver={() => this.props.onMouseOver ? this.props.onMouseOver({ maximum, minimum, median, quartile1, quartile3, day }) : null}
      ...
      <div style={{ textAlign: 'center', padding: '15px', width: '100%', height: 'calc(100% - 60px)' }}>
        <svg width={'100%'} height={'100%'}>
          <G x={0} y={5}>
            ...
            <BottomAxis scale={this.xScale} y={this.props.height - BoxPlot.axisXSize} x={BoxPlot.axisYSize} />
          </G>
        </svg>
      </div>
    );
