...
export const Container = styled.section`
  height: 100%;
  padding: ${(props) => (props.theme && props.theme.app.padding ? props.theme.app.padding : '0px')};
`;
      ...
      <Section10>
        <FloatLabel>
          <StockInput
            ...
            onBlur={(event) => (this.props.onBlur ? this.props.onBlur(event.target.value) : null)}
          />
          <StockLabel>{this.props.placeholder}</StockLabel>
        </FloatLabel>
      </Section10>
    );
  }
...
export class AreaField extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({ value: nextProps.value || nextProps.noValue });
  }
      ...
      <Section10>
        <FloatLabel>
          <Textarea
            type="text"
            placeholder={this.props.placeholder}
            onChange={({ target }) => this.setState({ value: target.value || this.props.noValue })}
            value={this.state.value}
            onBlur={(event) => (this.props.onBlur ? this.props.onBlur(event.target.value) : null)}
          />
          <StockLabel>{this.props.placeholder}</StockLabel>
